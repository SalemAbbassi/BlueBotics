report 50226 "BBX Proforma Transfer"
{
    WordLayout = './BlueBotics Proforma Transfer.docx';
    Caption = 'BlueBotics Proforma Transfer';
    DefaultLayout = Word;
    ...
    dataset
    {
        dataitem(CopyLoop; Integer)
        {
            ...
            dataitem("Transfer Shipment Header"; "Transfer Shipment Header")
            {
                ...
                column(No_TransferShipHeader; ReplaceString("No."))
                {
                }
                column(ExterDocNo_Header; "External Document No.")
                {
                }
                column(Order_No_Header; "Transfer Order No.")
                {
                }
                ...
                dataitem("Transfer Shipment Line"; "Transfer Shipment Line")
                {
                    ...
                    {
                        /*AutoFormatExpression = "Transfer Shipment Header"."Currency Code";
                        AutoFormatType = 2;*/
                    }
                    ...
                    begin
                        if RecGItem.Get("Transfer Shipment Line"."Item No.") then begin
                        ...
                        TxtGLineAmount := BlankZeroWithCurrencyCode(Quantity * "BBX Value");

                        DecGTotalAmount += Quantity * "BBX Value";
                        ...
                        TxtGUnitPrice := Format("BBX Value", 0, '<Integer Thousand><Decimals,3>');
                        //end;
                ...
                dataitem(InventoryCommentLine; "Inventory Comment Line")
                {
                    DataItemLinkReference = "Transfer Shipment Line";
                    DataItemTableView = where("Document Type" = const(2));
                    DataItemLink = "No." = field("Document No.");
                    column(Comment_InventoryCommentLine; Comment)
                    {
                    }
                }
                dataitem("Extended Text Line"; "Extended Text Line")
                {
                    ...
                    begin
                        ...
                        if RecGCountryRegion.Get("Transfer Shipment Header"."Trsf.-to Country/Region Code") then
    ...
    var
        DocumentTitleLbl: Label 'PROFORMA TRANSFER';
    ...
    begin
        with TransferShipmentHeader do
            FormatAddr.FormatAddr(TransferToAddr, "Transfer-to Name", "Transfer-to Name 2", "Transfer-to Contact", "Transfer-to Address", "Transfer-to Address 2",
    ...
    begin
        TxtRInvoivingAddress := StrSubstNo('%1, %2, %3, %4, %5', TransferToAddr[1], TransferToAddr[2], TransferToAddr[3], TransferToAddr[4], TransferToAddr[5]);
        if TransferToAddr[6] <> '' then
            TxtRInvoivingAddress := StrSubstNo('%1, %2, %3, %4, %5, %6', TransferToAddr[1], TransferToAddr[2], TransferToAddr[3], TransferToAddr[4], TransferToAddr[5], TransferToAddr[6]);

        if TransferToAddr[7] <> '' then
            TxtRInvoivingAddress := StrSubstNo('%1, %2, %3, %4, %5, %6, %7', TransferToAddr[1], TransferToAddr[2], TransferToAddr[3], TransferToAddr[4], TransferToAddr[5], TransferToAddr[6], TransferToAddr[7]);

        if TransferToAddr[8] <> '' then
            TxtRInvoivingAddress := StrSubstNo('%1, %2, %3, %4, %5, %6, %7, %8', TransferToAddr[1], TransferToAddr[2], TransferToAddr[3], TransferToAddr[4], TransferToAddr[5], TransferToAddr[6], TransferToAddr[7], TransferToAddr[8]);
