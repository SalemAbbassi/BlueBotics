report 50205 "BBX Sales order conf"
{
    WordLayout = './BlueBotics Sales Order confirmation.docx';
    Caption = 'BlueBotics Sales Order confirmation';
    DefaultLayout = Word;
    ...
    dataset
    {
        dataitem(Header; "Sales Header")
        {
            ...
            column(BlanketOrderNo; GetBlanketOrderNo(Header))
            {
            }
            column(BBXCourierAccount_Header; "BBX Courier Account")
            {
            }

            column(VATRegestrationNoCaption; RecGCustomer.FieldCaption("VAT Registration No.") + ':')
            {
            }
            column(ShippingAgentCode_Header; RecGShippingAgent.Name)
            {
            }
            column(VATRegestrationNo; RecGCustomer."VAT Registration No.")
            {
            }
            ...
            column(YourPurchaseNoCaption; YourPurchaseNoCaptionLbl)
            {
            }
            ...
            column(ShipmentMethodDescription_Lbl; StrSubstNo('%1:', ShptMethodDescLbl))
            {
            }
            column(Shipment_Lbl; ShipmentLbl)
            {
            }
            column(ShipmentDate; Format("Shipment Date", 0))
            {
            }
            ...
            column(PaymentTermsDescription_Lbl; StrSubstNo('%1:', PaymentTermsDescLbl))
            {
            }
            ...
            column(DocumentDate_Lbl; StrSubstNo('%1:', DocumentDateLbl))
            {
            }
            ...
            column(QuoteNo_Lbl; StrSubstNo('%1:', QuoteNOCaptionLbl))
            {
            }
            ...
            column(OurReference; RecGUserSetup."BBX Full Name")
            {
            ...
            column(Expected_Delivery_Date; format("Promised Delivery Date", 0))
            {

            }

            column(ExpectedDeliveryDate_Lbl; ExpectedDeliveryDateLbl)
            {
            ...
            dataitem(Line; "Sales Line")
            {
                ...
                DataItemTableView = SORTING("Document No.", "Line No.") where("DBE Task Billing Type" = filter(<> "Milestone Task"));
                UseTemporary = true;
            ...
            dataitem("Extended Text Line"; "Extended Text Line")
            {
                ...
                trigger OnPreDataItem()
                begin
                    if not BooGPrintStrandardText then
                        CurrReport.break();
            ...
            dataitem(TransportFeeText; "Extended Text Line")
            {
                column(TransportFeeText_Text; Text)
                {
                }
                trigger OnPreDataItem()
                var
                ...
                begin
                    RecLStandardText.SetRange("BBX Transport Fee Text", true);
                    if RecLStandardText.FindFirst() then begin
            ...
            dataitem(Totals; "Integer")
            {
                ...
                column(TotalAmountIncludingVAT; TxtGTotalAmountInclVAT)
                {
                    AutoFormatExpression = Header."Currency Code";
                    AutoFormatType = 1;
                }
            ...
            var
                ...
                IntLCount: Integer;
                RecLSalesHeaderTmp: Record "Sales Header" temporary;
                IntLI: Integer;
            begin
                FirstLineHasBeenOutput := false;
                Clear(Line);
                ...
                //DBE
                RecLSalesHeaderTmp.init;
                RecLSalesHeaderTmp.TransferFields(Header);
                IF RecLSalesHeaderTmp."Bill-to Name 2" <> '' then begin
                    RecLSalesHeaderTmp."Bill-to name" := RecLSalesHeaderTmp."Bill-to name 2";
                    RecLSalesHeaderTmp."Bill-to name 2" := '';
                end;
                FormatAddr.SalesHeaderBillTo(CustAddr, RecLSalesHeaderTmp);
                clear(TxtGDeliveryAddress);
                For intli := 1 to 8 do begin
                    if ShipToAddr[intli] <> '' then
                        TxtGDeliveryAddress += ShipToAddr[intli] + ',';
                end;
                ...
                IF ApprovalEntry.FINDSET then
                    repeat
                        ...
                        IF IntLCount = 1 then begin
                            if not RecGUserSetup1.Get(ApprovalEntry."Approver ID") then
                                RecGUserSetup1.Init()
                        end ELSE
                            IF IntLCount = 2 then begin
                                if not RecGUserSetup2.Get(ApprovalEntry."Approver ID") then RecGUserSetup2.Init();
                            end;
                    until (IntLCount = 2) OR (ApprovalEntry.Next = 0);

                if not RecGUserSetup1.IsEmpty then
                    CalcSignature(RecGUserSetup1, RecGTenantMedia1);
                if not RecGUserSetup2.IsEmpty then
                ...
                BooGPrintStrandardText := Header."BBX Partner";
                If BooGPrintStrandardText then begin
                ...
                if not RecGUserSetup.Get("BBX Project Manager") then RecGUserSetup.Init();
                if not RecGShippingAgent.Get("Shipping Agent Code") then RecGShippingAgent.Init();
    ...
    var
        ...
        PageLbl: Label 'Page';
        PaymentTermsDescLbl: Label 'Terms of payment';
        PaymentMethodDescLbl: Label 'Payment Method';
        PostedShipmentDateLbl: Label 'Shipment Date';
        SalesInvLineDiscLbl: Label 'Discount %';
        ShipmentLbl: Label 'Shipment';
        ShiptoAddrLbl: Label 'Delivery address :';
        ShptMethodDescLbl: Label 'Terms of delivery';
        ...
        YourPurchaseNoCaptionLbl: Label 'Your purchase order:';
        ...
        ExpectedDeliveryDateLbl: label 'Expected delivery date:';
        ...
        TxtGIntermediateAmount: TExt;
        DecGIntermediateAmount: Decimal;
        BooGPrintStrandardText: Boolean;
    ...
    local procedure GetBlanketOrderNo(RecPSalesHeader: Record "Sales Header"): Code[20]
    var
        RecLSalesLine: Record "Sales Line";
    begin
        RecLSalesLine.SetRange("Document Type", RecPSalesHeader."Document Type");
        RecLSalesLine.SetRange("Document No.", RecPSalesHeader."No.");
        if RecLSalesLine.FindSet() then
            repeat
                if RecLSalesLine."Blanket Order No." <> '' then
                    exit(RecLSalesLine."Blanket Order No.")
