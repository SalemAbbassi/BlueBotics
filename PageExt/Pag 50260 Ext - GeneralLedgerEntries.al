pageextension 50260 "BBX General Ledger Entries" extends "General Ledger Entries"
{
    layout
    {
        addafter(Amount)
        {
            field(OriginalAmount; DecGOriginalAmount)
            {
                ApplicationArea = all;
                Caption = 'Original Amount';
                Editable = false;
            }
            field(OriginalCurrency; CodGOriginalCurrencyCode)
            {
                ApplicationArea = all;
    ...
    begin
        CodGOriginalCurrencyCode := Rec.GetOriginalCurrencyCode();
        DecGOriginalAmount := Rec.GetOriginalAmount();
    end;

    var
        CodGOriginalCurrencyCode: Code[10];
        DecGOriginalAmount: Decimal;
    /*
